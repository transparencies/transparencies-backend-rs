...
impl<'a, C> Default for GitClient<'a, C>
where C: Client<'a> + Default
{
    fn default() -> GitClient<'a, C> {
        GitClient::new()
    }
}

impl<'a, C: Client<'a>> GitClient<'a, C> {
    /// Create a new client with an existing client
    pub fn with_client(client: C) -> GitClient<'a, C> {
        GitClient {
            client,
            _pd: std::marker::PhantomData::default(),
        }
    ...
    /// Create a new [`HelixClient`] with a default
    /// [`HttpClient`][crate::HttpClient]
    pub fn new() -> GitClient<'a, C>
    where C: Default {
        let client = C::default();
        GitClient::with_client(client)
    }
