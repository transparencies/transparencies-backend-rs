...
use crate::{
    domain::api_handler::{client::ApiClient, response::aoc_ref::RefDataLists},
    server::{
...
pub fn matchinfo(
    ...
    // Basically a filter that listens on all endpoints which just wraps and forwards
    // our aoe_net_client to make it reusable between requests to our API
    // This is warp specific to give the client as context to our `return_matchinfo_to_client`
    // function
    let aoe_net_client_filter = warp::any().map(move || aoe_net_client.clone());

    // A filter that wraps the `in-memory DB` of aoc_reference_data so we can have it as a context
    // in data processing
